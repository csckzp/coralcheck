...
impl GrammarGraph {
    ...
    fn transform_rules(&mut self, rules: &mut Vec<pest_meta::ast::Rule>) {
        ...
        // Append new rules in sorted (deterministic) order so that the
        // canonical grammar tables are independent of HashMap iteration order.
        let mut sorted_new: Vec<_> = new_rules.into_iter().collect();
        sorted_new.sort_by(|(a, _), (b, _)| a.cmp(b));
        for (name, expr) in sorted_new {
